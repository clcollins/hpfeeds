...
class FeedConn(EventGen):
    ...
    def auth(self, ident, hash):
        p = self.db.query('hpfeeds.auth_key',
                          {'identifier': str(ident)}, limit=1)
    ...
    def checkauth(self, r, hash):
        if len(r) > 0:
            akobj = r[0]
            akhash = hashlib.sha1(
                '{0}{1}'.format(self.rand, akobj['secret'])).digest()
            if akhash == hash:
                self.pubchans.update(akobj.get('publish', []))
                self.subchans.update(akobj.get('subscribe', []))
                self.idents.add(akobj['identifier'])
                logging.info(
                    'Auth success by {0}.'.format(akobj['identifier']))
            else:
                self.error('authfail.')
                logging.info(
                    'Auth failure by {0}.'.format(akobj['identifier']))
        else:
            self.error('authfail.')
        ...
        try:
            for opcode, data in self.fu:
                if opcode == OP_PUBLISH:
                    rest = buffer(data, 0)
                    ident, rest = rest[1:1 + ord(rest[0])], buffer(rest,
                                                                   1 + ord(
                                                                       rest[
                                                                           0]))
                    chan, rest = rest[1:1 + ord(rest[0])], buffer(rest,
                                                                  1 + ord(
                                                                      rest[0]))
                    ...
                    rest = buffer(data, 0)
                    ident, chan = rest[1:1 + ord(rest[0])], rest[
                                                            1 + ord(rest[0]):]
                    ...
                    checkchan = chan
                    if chan.endswith('..broker'): checkchan = \
                    chan.rsplit('..broker', 1)[0]
                    ...
                    rest = buffer(data, 0)
                    ident, hash = rest[1:1 + ord(rest[0])], rest[
                                                            1 + ord(rest[0]):]
    ...
    def msginfo(self):
        return self.msghdr(OP_INFO,
                           '{0}{1}{2}'.format(chr(len(FBNAME) % 0xff), FBNAME,
                                              self.rand))
    ...
    def msgpublish(self, ident, chan, data):
        return self.msghdr(OP_PUBLISH,
                           struct.pack('!B', len(ident)) + ident +
...
class FeedBroker(object):
    ...
    def initdb(self):
        self.db = MongoConn(host=config.MONGODB_HOST, port=config.MONGODB_PORT)
    ...
    def _dbclose(self, e):
        logging.critical(
            'Database connection closed ({0}). Exiting.'.format(e))
        unloop()
    ...
    def _subscribe(self, c, chan, ident):
        logging.debug(
            'broker subscribe to {0} by {2} @ {1}'.format(chan, c.addr, ident))
